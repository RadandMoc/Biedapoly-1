...
namespace Biedapoly
{
    public class Methods
    {
         ...
         * RHA - niszczenie domu każdego poziomu (poza hotelem)
         * RHO - niszczenie hotelu do domu 4-ro poziomowego
         * GPM - dostawanie pieniedzy z zastawu
         * PPM - Placenie za odzyskanie zastawu (*)
        ...
        static public void BonusesFromHeroToPlayer(Heroes hero, Player player) //do testu obowiązkowo
        {
            player.PlayerBonuses = hero.HeroesBonuses;
            string key, val, bin;
            foreach (var item in hero.HeroesBonuses)
            {
                key = item.Key;
                val = item.Value;
                if (val != "" & (!player.PlayerBonuses.TryGetValue(key, out bin)))
                    player.PlayerBonuses[key] = val;
                else if (val != "")
                    player.PlayerBonuses[key] = Methods.OverridingValue(player.PlayerBonuses[key], val);
            }
        }
        ...
        static public Map MakeStudentpolyMap()
        {
            ...
            Map studentpoly = new Map("Studentpoly", dicOfTerritories, defaultMoney, prospect, socialTreasury, moneyForPrison, alliances, moneyForStart, rings, "040", "010", bank, "000");
            return studentpoly;
        }
